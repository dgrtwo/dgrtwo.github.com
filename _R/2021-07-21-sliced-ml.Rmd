...
In this post, I'll share some of what I've learned about using R for competitive modeling. (Like I said I'm new to this, so I'm sure most of the post is old news for Kaggle experts!)

### R's tidymodels packages are a new "pit of success"

For years Python has had a solid advantage in tooling for machine learning ([scikit-learn](https://scikit-learn.org/) is just dynamite), but R has been catching up fast. The tidyverse team at RStudio has been developing the tidymodels suite of packages, which plays well with the tidyverse tools that I like. This was the first time I'd really gotten in-depth experience with them, and I was mostly- though not entirely- thrilled.
...
![http://varianceexplained.org/images/sliced/cross-validating-feature-meme.png](Meme of a mother ignoring one of her children who's struggling swimming. Text: "Cross validating model fitting" versus "Cross validating feature engineering.")

The recipes package makes this explicit: a recipe has to be trained on data (with `prep()`) before it is applied to a dataset (with `bake()`). By combining the step with a model specification into a `workflow` object, you encapsulate the entire start-to-end process of data cleaning.

Each of the other packages in tidymodels also play a role in the tidymodels "pit of success":

* **Cross validation (rsamples)**: Cross validation is one of the key ways to avoid overfitting, especially when choosing models and choosing hyperparameters.
...
* **Trying different models (parsnip)**: I really like not having to deal with each modeling package's idiosyncrasies of inputs and outputs; less time reading docs means more time working with my model and data.
...
Some of the resistance I've seen to tidymodels comes from a place of "This makes it too *easy*- you're not thinking carefully about what the code is doing!" But I think this is getting it backwards. By taking the burden of writing procedural logic, I mostly got to focus on scientific and statistical questions about my data and model. 
...
But the great news is that the team is continuing to improve it! I opened two GitHub issues ([parsnip](https://github.com/tidymodels/parsnip/issues/513) and [workflows](https://github.com/tidymodels/workflows/issues/108)) in the course of my SLICED competition (and talked through them with my old colleague [Julia](https://juliasilge.com/), who works on the tidymodels packages full time), and by the time I reached my second lap they were already fixed!
...
I know I'm just about the last person to appreciate this, but it was remarkable how powerful xgboost was. When I was practicing for the competition, I honed in on a process early on. Something like:
...
This meant we got to learn as a community (and joke around as well).
...
My competitor [Jesse Mostipak](https://twitter.com/kierisi) came up with a brilliant schtick of showing memes on screen while models were training:
...
For instance, here's a (lightly adapted) bit of model-fitting code from [episode 7](https://github.com/dgrtwo/data-screencasts/blob/master/ml-practice/ep7.Rmd).

```{r eval = FALSE}
train_10fold <- train %>%
  vfold_cv(10)

xg_rec <- recipe(churned ~ ., data = train) %>%
  step_mutate(avg_transaction_amt = total_trans_amt / total_trans_ct) %>%
  step_impute_mean(all_numeric_predictors()) %>%
  step_rm(id, income_category, education_level) %>%
